...
cd $BASE
NAME=$(ls *.jar 2>/dev/null)
NAME=${NAME%.jar}
ACTION=$1
APP_LOGS=${BASE}/logs
APP_PID=${BASE}/${NAME}.pid
CONFIG_URL=http://username:password@10.17.14.34:8709
CONFIG_ACTIVE=prod
SERVER_PORT=8080
...
JAVA_OPTIONS=("-Djava.io.tmpdir=/var/tmp" "-Duser.timezone=Asia/Shanghai")

CONFIG_OPTIONS=("--spring.profiles.active=$CONFIG_ACTIVE" "--server.port=$SERVER_PORT" "--spring.cloud.config.uri=$CONFIG_URL")
...
start() 
{
      ...
      then
        ...
        exit 0
         ...
         nohup $JAVA ${JAVA_OPTIONS[*]} -jar ${NAME}.jar ${CONFIG_OPTIONS[*]} > /dev/null &
         disown $!
         echo $! > "$APP_PID"
